...
#自己加的
from mmaction.models.backbones import action
    ...
    def __init__(self,
        ...
        for i in range(num_levels):
            ...
            layers += [TemporalBlock(in_channels, out_channels, kernel_size, stride=1, dilation=dilation_size,
                                     padding=(kernel_size - 1) * dilation_size, dropout=dropout), nn.AvgPool1d(2)]
...
class Bottleneck3d(nn.Module):
    ...
    def __init__(self,
        ...
        '''
        conv = weight_norm(nn.Conv3d(inplanes, planes, conv1_kernel_size, stride=(self.conv1_stride_t, self.conv1_stride_s,
                    self.conv1_stride_s), padding=conv1_padding, bias=False))
        action_module = Action(conv, n_segment=this_segment, shift_div=n_div)
        bn = nn.BatchNorm3d(32, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        requires_grad = norm_cfg.pop('requires_grad', True)
        for param in self.bn.parameters():
            param.requires_grad = requires_grad
        relu1 = nn.ReLU(inplace=True)
        self.conv1 = nn.Sequential(conv, bn, relu1)
        '''
