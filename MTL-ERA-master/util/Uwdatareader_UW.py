...
class Dataset_with_REBA(data.Dataset):
    def __init__(self, seqlist, history=90):
        ...
        for seq in seqlist:
            ...
            self.UWIOMheatmap = np.load(r"/home/zeng_jy/mmaction2-master/UWIOMheatmap/S001C001P001R001A0{}_rgbresults.npy".format(seq),
                                      allow_pickle=True, encoding="latin1").item()
            self.UWIOMheatmap_arr = self.UWIOMheatmap['imgs'].squeeze()
...
#        pose = self.poselist[index]
        UWIOMheatmapdata = self.UWIOMheatmap_arr
...
#        return np.double(pose), np.double(np.asarray(lab)), np.double(reba)
        return UWIOMheatmapdata, np.double(np.asarray(lab)), np.double(reba)
